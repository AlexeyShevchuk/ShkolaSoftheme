...
namespace Delegates
{
    ...
    {
        [DataMember] public List<MobileAccount> List { get; set; } = new List<MobileAccount>();
        ...
        public void Route(int fromMobileId, int toMobileId, Action action)
        {
            ...
            {
                var fromMobile = List.Find(x => x.Id == fromMobileId);
                var toMobile = List.Find(x => x.Id == toMobileId);
                switch (action)
                {
                    case Action.SMS:
